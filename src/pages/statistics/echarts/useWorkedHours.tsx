...
const McWorkedHours: FC<IProps> = ({ uuid, type, handlerChange }) => {
  ...
  useEffect(() => {
    const url = uuid
      ? `/sysRadioOperational/worked_hours/${type}?radioUuid=${uuid}`
      : `/sysRadioOperational/worked_hours/${type}`;
    const wait = fetch.get<ManageResponse>(url);
    Promise.resolve(wait).then(response => {
      const result = response.data;
      if (result.status === 1 && result.data) {
        setTotal(result.data.details);
        handlerChange(result.data.workedHours);
      }
    });
