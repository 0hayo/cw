...
const xcode = {
  ...
  readServer: async (dir: string, uuid: string): Promise<{}> => {
    const { data } = await fetch.get<MstResponse>(`/sysDatagramAttachment/${uuid}/code`);
    console.log("xcode dataResponse", data);
  ...
  saveServer: async (dir: string, telegram: McTelegram): Promise<void> => {
    let { data } = await fetch.get<MstResponse>(`/sysDatagramAttachment/${dir}/code`);
    ...
    if (data.data?.content) {
      ...
      console.log("update telegram data=", postParam);
      ...
      console.log("insert telegram data=", postParam);
      await fetch.post<MstResponse>("/sysDatagramAttachment/", JSON.stringify(postParam));
    }
  },

  updateServer: async (dir: string, telegram: McTelegram): Promise<void> => {
    const postParam = {
      content: JSON.stringify(telegram),
      datagramUuid: dir,
      type: "code",
